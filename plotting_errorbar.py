...
with open('SWim BEttwer_CC_timelag_total.csv', newline='\n' ) as inputfile:
   cc = list(csv.reader(inputfile))  
with open('SWim BEttwer_CC_PEAK_total.csv', newline='\n' ) as inputfile:
   cc_peak = list(csv.reader(inputfile)) 
with open('SWim BEttwer_MI_total.csv', newline='\n' ) as inputfile:
...
with open('Forward_timelag_fix__CC_timelag_total.csv', newline='\n' ) as inputfile:
   control_cc = list(csv.reader(inputfile))
with open('Forward_timelag_fix__CC_PEAK_total.csv', newline='\n' ) as inputfile:
   control_cc_peak= list(csv.reader(inputfile))   
with open('Forward_timelag_fix__MI_total.csv', newline='\n' ) as inputfile:
...
def plot_all(a, y):
    ...
    plt.ylabel(y) 
    plt.title(title)
    plt.savefig(title + y + "individ.png")
    plt.show()
...
def plot_compare(control, a, control_std, a_std,y):
    x_val = []
    ...
    plt.savefig(savename + y + "comparison.png")
    plt.show()
...
def plot_compare_two_axis(control, a, control_std, a_std,y):
    ...
    plt.savefig(savename + "comparison.png")
    plt.show()
def plot_errorbars_single(control, control_std,y, save = False):
    ...
    if save == True:
        plt.savefig(savename + y + "single.png")
    plt.show()    
    return
...
#plot_compare(control_cc_avg, cc_avg,control_cc_std, cc_std,y = 'Time lag to peak cross correlation (s)')
#plot_compare(control_cc_peak_avg, cc_peak_avg, control_cc_peak_std, cc_peak_std, y = 'Normalized peak cross correlation')
#plot_compare(control_mi_avg, mi_avg, control_mi_std, mi_std, y = 'Normalized mutual information')
...
def heatmap():
    ...
    for i in range(13):
        heatmap.append(i)
    for i in range(13):
        input_name = 'n2_SWIMMIN' + str(i) +'_CC_PEAK_total.csv'
    ...
    for i in range(13):
        x_axis.append(i+1) 
    seaborn.heatmap(heatmap, vmax =1,xticklabels = x_axis, yticklabels = x_axis)
    plt.xlabel('Segment number')  
    plt.ylabel('Segment number')         
       
    plt.savefig("heatmap_SWIM_CC.png")
    return heatmap
heatmapp = heatmap()
